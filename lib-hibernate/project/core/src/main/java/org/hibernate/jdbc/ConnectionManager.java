...
public class ConnectionManager implements Serializable {
	...
	/**
	 * If a per-session datasource is set at session create time this contains the source to
	 * use to allocate connections from. It overrides the existing Hibernate connection creation
	 * crud (the SessionFactory's ConnectionProvider).
	 * jal 20090605 This is required to allow setting the red_environment.set_user on EVERY connection allocated,
	 */
	 ...
	 * @param dataSource when not null, the dataSource to allocate connections from.
	 */
	...
	private void openConnection() throws HibernateException {
		...
		try {
			// 20090604 jal Fix voor alloceren connectie via per-session datasource
			connection = internalCreateConnection();
		}
	...
	/**
	 * Close connection. 20090605 jal Fix for per-session ds
	 * @param c
	 * @throws SQLException
	 */
	public void			internalCloseConnection(final Connection c) throws SQLException {
		if(m_dataSource != null)
			c.close();
		else
			factory.getConnectionProvider().closeConnection(c);
	}

	public Connection	internalCreateConnection() throws SQLException {
		if(m_dataSource != null)
...
//			factory.getConnectionProvider().closeConnection( connection );	jal 20090605 fix for per-session ds
			connection = null;
		}
		catch (SQLException sqle) {
			throw JDBCExceptionHelper.convert(
