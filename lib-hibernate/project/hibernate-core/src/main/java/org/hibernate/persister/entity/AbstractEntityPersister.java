			...
			for ( int i = 0; i < types.length; i++ ) {
				if ( !propertySelectable[i] ) {
					...
					if ( propertyIsDeferred && sequentialSelectEmpty ) {
                        ...
                        // jal 20090818 Added try/catch to catch conversion errors and log a proper message for them, including record, column and property name damnit.
                        try {
                        	values[i] = types[i].hydrate(propertyResultSet, cols, session, object);
                        } catch(SQLException x) {
                            StringBuilder sb = new StringBuilder(128);
                            sb.append("Failed to convert property '");
                            sb.append(propNames[i]);
                            sb.append("' of '");
                            sb.append(entityMetamodel.getName());
                            sb.append("'\n- column(s) ");
                            ...
                            throw new RuntimeException(sb.toString(), x);
                        }
	...
	protected boolean update(
		...
		try {
			...
			try {
				 ...
				 * record. After the first of these have been executed the second one has stale data in
				 * it's old copy of the generated fields. If the data is checked by a database trigger
				 * this will cause the update to fail.
				 *
				 * The fix is to check which fields are generated, and replace the "old" values for
				 * those fields with the actual (current) values in the persistent object.
				 */
				if(hasUpdateGeneratedProperties() || hasInsertGeneratedProperties()) {
					ValueInclusion[] inar = getPropertyInsertGenerationInclusions();
					ValueInclusion[] upar = getPropertyUpdateGenerationInclusions();
					if(inar.length != upar.length || inar.length != fields.length)
						throw new IllegalStateException("Bad return from one of the GenerationInclusion thingies");

					for(int i = fields.length; --i >= 0;) {
						if(inar[i] == ValueInclusion.NONE && upar[i] == ValueInclusion.NONE)
							continue;

						//-- Need to obtain field value from actual instance.
						fields[i] = getPropertyValue(object, i, session.getEntityMode());
						//System.out.println("    ... abstract.update: got actual value for "+getPropertyNames()[i]+" = "+fields[i]);
					}
				}
				//-- hhh-2588:3 fix end
