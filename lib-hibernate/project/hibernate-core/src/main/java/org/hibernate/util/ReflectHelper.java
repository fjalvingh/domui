...
public final class ReflectHelper {
	...
	/**
	 * EXPENSIVE
	 * If the "getId" method of an entity is part of a generic interface on an antity, say:
	 * <pre>
	 * interface IIdentifyable<T> {
	 *   T getId();
	 * }
	 * </pre>
	 * then implementations of this interface suffer the effect of the "type erasure" kludge in Java. Because
	 * the T in the interface is erased the "real" prototype for getId() that <i>must be generated by the compiler</i>
	 * is:
	 * <pre>
	 *  public Object getId();
	 * </pre>
	 *
	 * even though the real implementation in {@code class Relation implements IIdentifyable<Long>} means we must have:
	 * <pre>
	 *  public Long getId();
	 * </pre>
	 *
	 * The compiler "fixes" this by generating both 8-/.
	 *
	 * This method, when passed the "original" Method (Long getId()) will try to find if there also
	 * exists an "Object getId()" and if so will return it. It needs to walk all Methods to do this, because to
	 * add insult to injury the Class.getMethod() call cannot find it (because the methods are technically duplicates).
	 *
	 * @param original
	 * @return
	 */
	public static Method findSyntheticGenericIdMethod(Method original) {
		if(null == original)
			return null;
		int plen = original.getParameterTypes().length;
		Class< ? > clz = original.getDeclaringClass();
		for(Method m : clz.getMethods()) {
			if(m != original && m.getName().equals(original.getName())) {
				if(m.getParameterTypes().length == plen) {
					if(m.getReturnType() == Object.class) {
						//-- Compare parameter arrays
						Class<?>[] aar = original.getParameterTypes();
						Class<?>[] bar = m.getParameterTypes();
						boolean found = true;
						for(int i = aar.length; --i >= 0;) {
							if(aar[i] != bar[i]) {
								found = false;
								break;
							}
