...
public class MarkerImagePart implements IBufferedPartFactory {
	...
	public void generate(PartResponse pr, DomApplication da, Object key, ResourceDependencyList rdl) throws Exception {
		...
		try {
			BufferedImage bi = PartUtil.loadImage(da, PageContext.getRequestContext().translateResourceName(DomUtil.isBlank(sipKey.getIcon()) ? DEFAULT_ICON : sipKey.getIcon().trim()), rdl);
	...
	public static String getBackgroundImage(String icon, String caption, String color) {
		String url = PageContext.getRequestContext().getRelativePath(getURL(icon, caption, color));
		return url;
	}

	public static String getBackgroundImage(String icon, String caption, String color, String font, int size, String spec) {
		String url = PageContext.getRequestContext().getRelativePath(getURL(icon, caption, color, font, size, spec));
		return url;
	}
