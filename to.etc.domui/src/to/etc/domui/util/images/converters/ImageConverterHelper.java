...
public class ImageConverterHelper {
	...
	public static void main(String[] args) {
		try {
			...
			System.out.println("Result: "+h.getTarget().getSource()+", mime="+h.getTarget().getMime()+", pages="+h.getTarget().getData().getPageCount()+"; p0.size="+h.getTarget().getData().getPage(0).getWidth()+"x"+h.getTarget().getData().getPage(0).getHeight());
		} catch(Exception x) {
			x.printStackTrace();
		}
	}
