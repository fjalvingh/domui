...
public class HtmlFullRenderer extends NodeVisitorBase implements IContributorRenderer {
	...
	protected void renderThemeCSS() throws Exception {
		...
		String url = appendStyleSheetHash(ctx().getRelativePath(sheet), sheet);
	...
	/**
	 * Add a hash over a generated stylesheet's content to its URL, so that the browser may cache
	 * the sheet forever and still pick up every change: as soon as the generated content differs
	 * the URL differs too.
	 *
	 * <p>This is what makes the theme variant reach the browser at all for a sheet whose URL does
	 * not name the variant - an application stylesheet, which is compiled for the session's
	 * variant but is asked for as plain "css/style.scss". Its content is different per variant, so
	 * its hash is, and a switch from light to dark therefore asks for a URL the browser has not
	 * got. Without this the browser would keep serving the sheet of the variant that was left.</p>
	 *
	 * <p>The URL is returned unchanged when no part generates the sheet: a plain .css resource has
	 * a stable URL and nothing to hash.</p>
	 */
	@NonNull
	private String appendStyleSheetHash(@NonNull String url, @NonNull String rurl) throws Exception {
		String hash = calculateStyleSheetHash(rurl);
		if(null == hash)
			return url;
		return url + (url.indexOf('?') < 0 ? '?' : '&') + "$hash=" + hash;
	}

	/**
	 * Generate the sheet for the variant this page renders in and return a hash over its content,
	 * or null when it is not generated by a part.
	 */
	@Nullable
	private String calculateStyleSheetHash(String sheet) throws Exception {
		int qix = sheet.indexOf('?');                                            // A contributor may add parameters of its own; the part is asked for the path.
		String path = qix < 0 ? sheet : sheet.substring(0, qix);
		int slash = path.lastIndexOf('/');
		PageParameters pp = new PageParameters()
			.themeVariant(m_ctx.getThemeVariant().getVariantName())
			.browserVersion(ctx().getPageParameters().getBrowserVersion())
			.inputPath(path);
		pp.setUrlContextString(slash < 0 ? "" : path.substring(0, slash + 1));    // As the browser's request for the sheet will have it, so that both use the same part cache entry
		try {
			...
			LOG.debug("No part generates the stylesheet '" + path + "', rendering its URL without a content hash");
			return null;
		}
	}
	...
	public void renderLoadCSS(String path, String... options) throws Exception {
		...
		path = appendStyleSheetHash(ctx().getRelativePath(rurl), rurl);
