...
(function ($) {
	...
	var rmFormat = {
		...
		removeStyle: function(node) {
		  ...
		  } else if(this.options.rules.styleMSO) {
			  var txt = $(node).attr('style');
			  if(txt.search(/\-mso/) >= 0)
			    $(node).removeAttr('style');
		  }
		},
		...
		msWordMarkup: function (text) {
			...
			text = text.replace(/style="[^"]*"/gmi, "");	// jal 20120905 remove all style attributes
			...
			//-- jal 20120906 empty tag soup
			for(;;) {
				var ot = text;
				text = text.replace(/<p>[\s]*<span>[&nbsp;|\s]*?<\/p>/gi, "");	// <p><span> </p> crap
