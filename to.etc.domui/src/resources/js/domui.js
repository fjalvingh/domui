...
(function($) {
	...
	function executeXML(xml) {
		...
		if(rname == 'redirect') {
			...
			if(!$.browser.msie) {
				// -- jal 20130129 For large documents, redirecting "inside" an existing document causes huge problems,
				// the
...
/**
 * jQuery scroll overflow fixerydoo for IE7's "let's create huge problems by putting a scrollbar inside the scrolling
 * area" blunder. It locates all scrolled-in area's and adds 20px of padding at the bottom.
 */
(function($) {
		...
		return this.each(function() {
			if(this.scrollWidth > this.offsetWidth) {
				...
				if(this.scrollHeight <= this.offsetHeight) {
					// hide vertical scroller only if it is not needed after
...
(function($) {
	$.fn.setBackgroundImageMarker = function() {
		return this.each(function() {
			...
			// Wrap this in a try/catch block since IE9 throws "Unspecified
        ...
        WebUI,
        {
            ...
            pickle : function(scope, fn) {
	            ...
	            return function() {
		            ...
		            for( var i = 0, len = args.length; i < len; i++)
			            // Append all args added to pickle
			            nargs.push(args[i]);
		            for( var i = 0, len = arguments.length; i < len; i++)
			            // Append all params of the actual function after it
            ...
            getInputFields : function(fields) {
	            ...
	            for( var i = q1.length; --i >= 0;) {
		            ...
		            if(t.type == 'file')
			            continue;
		            if(t.type == 'hidden' && !t.getAttribute('s')) // All hidden input
			            // nodes are created
			            // directly in
			            // browser
			            // java-script and
            ...
            clicked : function(h, id, evt) {
	            ...
	            // e.preventDefault(); // jal 20110216 DO NOT PREVENTDEFAULT- it will
	            // disable checkbox enable/disable
            ...
            valuechanged : function(h, id) {
	            // FIXME 20100315 jal Temporary fix for bug 680: if a DateInput has a
	            // value changed listener the onblur does not execute. So handle it here
	            // too.... The fix is horrible and needs generalization.
            ...
            /**
			 * Handle enter key pressed on keyPress for component with onLookupTyping listener. This needs to be
			 * executed on keyPress (was part of keyUp handling), otherwise other global return key listener
			 * (returnKeyPress handler) would fire.
			 */
            onLookupTypingReturnKeyHandler : function(id, event) {
	            ...
	            if(isReturn) {
		            ...
		            // Do not call upward handlers too, we do not want to trigger on
		            // value changed by return pressed.
		            ...
		            var trNode = selectedIndex < 0 ? null : $(node.parentNode).children("div.ui-lui-keyword-popup").children("div").children("table")
		                .children("tbody").children("tr:nth-child(" + selectedIndex + ")").get(0);
		            if(trNode) {
            ...
            /*
			 * Executed as onkeyup event on input field that has implemented listener for onLookupTyping event. In case
			 * of return key call lookupTypingDone ajax that is transformed into onLookupTyping(done=true). In case of
			 * other key, lookupTyping funcion is called with delay of 500ms. Previuosly scheduled lookupTyping function
			 * is canceled. This cause that fast typing would not trigger ajax for each key stroke, only when user stops
			 * typing for 500ms ajax would be called by lookupTyping function.
			 */
            scheduleOnLookupTypingEvent : function(id, event) {
	            ...
	            var keyCode = WebUI.normalizeKey(event);
	            var isReturn = (keyCode == 13000 || keyCode == 13);
	            if(isReturn) { // handled by onLookupTypingReturnKeyHandler, just
		            // cancel propagation
		            event.cancelBubble = true;
		            if(event.stopPropagation)
			            event.stopPropagation();
	            ...
	            if(isDownArrowKey || isUpArrowKey) {
		            ...
		            if(selectedIndex < 0)
			            selectedIndex = 0;
		            var trNode = $(node.parentNode).children("div.ui-lui-keyword-popup").children("div").children("table").children("tbody")
		                .children("tr:nth-child(" + selectedIndex + ")").get(0);
		            if(trNode) {
			            trNode.className = "ui-keyword-popup-row";
		            }
		            var trNodes = $(node.parentNode).children("div.ui-lui-keyword-popup").children("div").children("table").children("tbody")
		                .children("tr");
		            if(trNodes.length > 0) {
			            ...
			            trNode = $(node.parentNode).children("div.ui-lui-keyword-popup").children("div").children("table").children("tbody")
			                .children("tr:nth-child(" + selectedIndex + ")").get(0);
			            if(trNode) {
            ...
            lookupPopupClicked : function(id) {
	            ...
	            if(selectedIndex < 0)
		            selectedIndex = 0;
	            var trNode = $(node.parentNode).children("div.ui-lui-keyword-popup").children("div").children("table").children("tbody").children(
	                "tr:nth-child(" + selectedIndex + ")").get(0);
	            if(trNode) {
            ...
            hideLookupTypingPopup : function(id) {
	            ...
	            if($.browser.msie) {
		            // IE kills event stack (click is canceled) when z index is set
		            // during onblur event handler... So, we need to postpone it a
		            // bit...
		            ...
		            // Other browsers dont suffer of this problem, and we can set z
		            // index instantly
		            node.parentNode.style.zIndex = node.style.zIndex;
	            }
            },
            ...
            showLookupTypingPopupIfStillFocusedAndFixZIndex : function(id) {
	            ...
	            if(qDivPopup.length > 0) {
		            ...
		            // z-index correction must be set manually from javascript (because
		            // some bug in IE7 -> if set from domui renders incorrectly until
		            // page is refreshed?)
	            ...
	            if(trNods && trNods.length > 0) {
		            for( var i = 0; i < trNods.length; i++) {
			            ...
			            // we need this jquery way of attaching events, if we use
			            // trNod.setAttribute("onmouseover",...) it does not work in IE7
			            $(trNod).bind("mouseover", {
			                nodeId : id,
			                trId : trNod.id
			            }, function(event) {
            ...
            /*
			 * In case of longer waiting for lookupTyping ajax response show waiting animated marker. Function is called
			 * with delay of 500ms from ajax.beforeSend method for lookupTyping event.
			 */
            ...
            lookupTyping : function(id) {
	            ...
	            // check for exsistence, since it is delayed action component can be
	            // removed when action is executed.
	            if(lookupField) {
		            // Collect all input, then create input.
		            var fields = new Object();
		            this.getInputFields(fields);
            ...
            handleError : function(request, status, exc) {
	            ...
	            if(txt.length == 0) {
		            // -- Firefox fix: if the page is unloading but a request is pending
		            // this may cause an status=ERROR for that page. Prevent us from
		            // then overwriting the new document....
		            if(status == "error")
			            return;
            ...
            handleErrorAsy : function(request, status, exc) {
	            ...
	            /*
				 * As usual there is a problem with error reporting: if the request is aborted because the browser
				 * reloads the page any pending request is cancelled and comes in here- but with the wrong error code of
				 * course. So to prevent us from showing an error message: set a timer to show that message 250 milli's
				 * later, and hope the stupid browser disables that timer.
				 */
	            setTimeout(function() {
		            ...
		            d.appendChild(document.createTextNode(WebUI._T.sysPollFailTitle)); // Server
		            // unreachable

		            d = document.createElement('div'); // Message content
		            ald.appendChild(d);
		            d.className = "ui-ioe-msg";
		            d.appendChild(document.createTextNode(txt)); // Server
		            // unreachable
		            ...
		            d.appendChild(document.createTextNode(WebUI._T.sysPollFailCont)); // Waiting
		            // for
		            // the
		            // server
		            // to
            ...
            /*
			 * IE/FF compatibility: IE only has the 'keycode' field, and it always hides all non-input like arrows, fn
			 * keys etc. FF has keycode which is used ONLY for non-input keys and charcode for input.
			 */
            ...
            focus : function(id) {
	            ...
	            if(n) {
		            if($.browser.isIE) {
			            ...
			            }, 100); // Due to IE bug, we need to set focus on timeout :( See
            ...
            showCalendar : function(id, withtime) {
	            ...
	            var params = {
	                inputField : inp,
	                eventName : 'click',
	                ifFormat : Calendar._TT[withtime ? "DEF_DATETIME_FORMAT" : "DEF_DATE_FORMAT"],
	                ...
	                showsTime : withtime,
	                timeFormat : "24",
	                electric : false, // jal 20110125 Fixes bug 885- do not update the
	                // field when moving to prevent firing the
	                // change handler.
	                step : 2,
	                position : null,
	                cache : false
	            ...
	            var cal = new Calendar(1, params.date, WebUI.onDateSelect, function(cal) {
		            cal.hide();
		            cal.destroy();
	            });
            ...
            toClip : function(value) {
	            if(window.clipboardData) {
		            ...
		            if(value.createTextRange) {
			            ...
			            var divinfo = '<embed src="$js/_clipboard.swf" FlashVars="clipboard=' + encodeURIComponent(value)
            ...
            /** ******************* Scheduler component ************************ */
            /**
			 * This handles item placement for a single calendar. It "discovers" the calendar divs using fixed css class
			 * names, and uses "extra" attributes generated on the root divs for the parts to get the calendar data.
			 * This handles overlapping appointments by dividing a full day lane into multiple "ways". A day lane starts
			 * with one "way". For every appointment added to a day we check if this appointment overlaps another
			 * appointment on this way[0]. If it does we create a next way (or use it if it already exists) and check if
			 * the appointment clashes there. We do this until we find a way that can accept the appointment without
			 * overlaps. We do this for all appointments. At the end we have a list of ways for every day. We now start
			 * rendering the appointments by going through them per way, then per appointment. For every appointment on
			 * a way we check if there is an appointment on a LATER way that overlaps. If not the width of the
			 * appointment includes all LATER ways that do not overlap. For this code to work no item may "overlap" a
			 * day. The server code takes care of splitting long appointments into multiple "items" here.
			 */
            ...
            /**
			 * None of the "standard" JS libraries like Rico or Prototype have code that actually <i>works</i> to get
			 * the actual <i>page or absolute</i> position of elements when scrolling is used. All of them
			 * unconditionally add scroll offsets to the relative positions but scrolling *will* cause items to become
			 * *invisible* because they are scrolled out of view. The calls here obtain a location for elements taking
			 * scrolling into account, and they will return null if the item is not visible at all.
			 */
            ...
            poll : function() {
	            ...
	            $.ajax({
	                ...
	                data : fields,
	                cache : false,
	                global : false, // jal 20091015 prevent block/unblock on polling
	                // call.
	                success : WebUI.handleResponse,
            ...
            /**
			 * Send Ajax request to the server every 2 minutes. This keeps the session alive. The response can contain
			 * commands to execute which will indicate important events or changes have taken place.
			 */
            ...
            fileUploadChange : function(e) {
	            ...
	            if(val) {
		            ...
		            if(li != -1) {
			            ...
			            // Allow upload of files without extensions when "*" filter is
			            // defined
			            ext = "";
		            }
	            ...
	            if(!iframe) {
		            if(jQuery.browser.msie && !WebUI.isNormalIE9plus()) { // MicroMorons
			            // report
			            // ie9 for
			            // ie7
			            // emulation
            ...
            /**
			 * Format a NLS message containing {0} and {1} markers and the like into a real message.
			 * 
			 * @param message
			 * @returns
			 */
            ...
            /**
			 * Called for ie upload garbage only, this tries to decode the utter devastating mess that ie makes from xml
			 * uploads into an iframe in ie8+. Sigh. The main problem with IE is that the idiots that built it mess up
			 * XML documents sent to it: when the iframe we use receives an xml document the idiot translates it into an
			 * html document- with formatted tags and minus signs before it to collapse them. Instead of just
			 * /rendering/ that they actually /generate/ that as the content document of the iframe- so getting that
			 * means you get no XML at all. This abomination was more or less fixed in ie9 - but of course they fucked
			 * up compatibility mode badly. Everything below ie9 ==================== Everything below ie9 has a special
			 * property "XMLDocument" attached to the "document" property of the window. This property contains the
			 * original XML that was used as an XML tree. So for these browsers we just get that and move on. IE9 in
			 * native mode ================== IE9 in native mode does not mess up the iframe content document, so this
			 * mode takes the path of all browsers worth the name. Since the original problem was solved the XMLDocument
			 * property no longer exists. IE9 in compatibility mode (IE7) =============================== This gets
			 * funny. In this mode the browser /still/ messes up the iframe's content model like the old IE's it
			 * emulates. But of course the XMLDocument property is helpfully removed - EVEN IN THIS MODE. Remember: this
			 * mode is also entered if you are part of a frameset or iframe that is part of an mode: the topmost
			 * frameset/page determines the mode for the entire set of pages - they are that stupid. I know no other
			 * "workaround" than the horrible concoction below; please turn away if you have a weak stomach.... In this
			 * case we get the "innerText" content of the iframe. This differers from innerHTML in that all html tags
			 * that IE added are removed, and only the text is retained. Since the html was generated by IE in such a
			 * way that the xml was presented to the user - this is actually most of our XML... But it contains some
			 * problems: - there is extra whitespace around it's edges, so we need to remove that.. - All tags start on
			 * a new line with a '-' sign before them... Remove all that... - The result will have the &amp; entity
			 * replaced by & because they are really stupid, again. So replace that too. The resulting thing can
			 * sometimes be parsed as XML and then processing continues. But it is far from perfect. The biggest problem
			 * is that the resulting xml has not properly reserved the original whitespace; this may lead to rendering
			 * problems. But as far as I know no other solution to this stupifying bug is possible. Please prove me
			 * wrong.
			 * 
			 * @param e
			 */
            ieUpdateUpload : function(e) { // Piece of crap
	            ...
	            if(iframe.contentWindow && iframe.contentWindow.document.XMLDocument) {
		            xml = iframe.contentWindow.document.XMLDocument; // IMPORTANT
		            // Fucking MS
		            // Crap!!!! See
            ...
            updateUpload : function(doc, ifr) {
	            ...
	            /*
				 * 20081015 jal De iframe zou verwijderd moeten worden, maar als ik dat doe onder Firefox dan blijft de
				 * browser in "busy" mode staan, met een rode enabled "stop" knop en een "busy" mouse.. Voor nu
				 * hergebruiken we de bestaande iframe dan maar en accepteren de verloren resources.
				 */
            ...
            /**
			 * Called at page unload time, this quickly tries to send an obituary to the server. This is currently
			 * unconditional but can later be augmented to send the obituary only when the browser window closes.
			 */
            ...
            isBrowserClosed : function(e) {
	            try {
		            ...
		            if(window.event) {
			            alert('wcy=' + window.event.clientY + ", wcx=" + window.event.clientX + ", dw=" + document.documentElement.clientWidth
            ...
            /**
			 * When mouse is downed on an item that is draggable. This moves to PREDRAG mode where the first move will
			 * create a visible representation of this node, ready for dropping.
			 */
	            ...
	            try {
		            if(WebUI._dragMode == 2) {
			            ...
			            if(dz) {
				            ...
				            WebUI._dragNode.style.display = '';// no drop zone, so
				            // restore the dragged
				            // item
			            }
		            }
            ...
            dragMouseMove : function(e) {
	            ...
	            WebUI._dragCopy.style.top = (e.clientY - WebUI._dragSourceOffset.y) + "px";
	            WebUI._dragCopy.style.left = (e.clientX - WebUI._dragSourceOffset.x) + "px";
            ...
            dragCreateCopy : function(source) {
	            ...
	            // If we drag a TR we need to encapsulate the thingy in a table/tbody to
	            // prevent trouble.
	            if(source.tagName != "TR") {
		            ...
		            t.appendChild(b);
		            b.innerHTML = source.innerHTML; // Copy tr inside tbody we just
		            // constructed
            ...
            /**
			 * Resets the dropzone timer. Called when in DRAGGING mode and the mouse moves, this resets any "open"
			 * dropzone indicators and resets the timer on which drop zone effects are done. This causes the dropzone
			 * indicator delay when moving the mouse.
			 */
            ...
            /**
			 * Fires when in DRAGGING mode and the mouse has not moved for a while. It initiates the rendering of any
			 * drop zone indicators if the mouse is above a drop zone.
			 */
            ...
            /**
			 * Gets or recalculates the list of possible drop targets and their absolute on-screen position. This list
			 * is used to determine if the mouse is "in" a drop target. The list gets cached globally in the WebUI
			 * object; if an AJAX request is done the list gets cleared.
			 */
            dropGetList : function() {
	            ...
	            for( var i = dl.length; --i >= 0;) {
		            ...
		            var tar = types.split(",");
		            def._types = tar;
		            def._drophandler = WebUI.findDropZoneHandler(drop.getAttribute('uidropmode'));
            ...
            dropTargetFind : function(x, y) {
	            ...
	            for( var i = dl.length; --i >= 0;) {
		            ...
		            if(x >= d._position.x && x < d._position.x + d._width && y >= d._position.y && y < d._position.y + d._height) {
            ...
            scrollRight : function(bRight) {
	            ...
	            var tabsTotalWidth = $('li:last', scrlNavig).width() + 8 /*
																			 * paddong = 8
																			 */
	                + $('li:last', scrlNavig).offset().left - $('li:first', scrlNavig).offset().left;
            ...
            recalculateScrollers : function(scrlNavigId) {
	            ...
	            var tabsTotalWidth = $('li:last', scrlNavig).width() + 8 /*
	                ...
	                + $('li:last', scrlNavig).offset().left - $('li:first', scrlNavig).offset().left;
	            var tabsVisibleWidth = $(scrlNavig).width() - 2 * $('.ui-stab-scrl-right', scrlNavig).width();
	            // WebUI.debug('debug1', 50, 150, 'width:' +
	            // ($('li:last',scrlNavig).width() + 8 +
	            // $('li:last',scrlNavig).offset().left -
	            // $('li:first',scrlNavig).offset().left));
	            // WebUI.debug('debug2', 50, 200, 'offsetX:' +
	            // ($('li:first',scrlNavig).offset().left - $('ul',
	            // scrlNavig).offset().left));
	            ...
	            if(tabsTotalWidth > tabsVisibleWidth) {
		            ...
		            // 20121115 btadic: we just want to render scroll buttons without
		            // arrows and actions. We don't want empty spaces.
            ...
            stretchHeight : function(elemId) {
	            ...
	            $(elem).siblings().each(function(index, node) {
		            // do not count target element and other siblings
		            // positioned absolute or relative to parent in
		            // order to calculate how much space is actually
		            // taken / available
		            if(node != elem && $(node).css('position') == 'static' && ($(node).css('float') == 'none' || $(node).css('width') != '100%' /*
																																				 ...
																																				 * width
																																				 */)) {
			            // In IE7 hidden nodes needs to be additionaly
			            // excluded from count...
	            ...
	            var elemDeltaHeight = $(elem).outerHeight(true) - $(elem).height(); // we
	            // need
	            // to
	            // also
	            // take
	            ...
	            if($.browser.msie && $.browser.version.substring(0, 1) == "7") {
		            // we need to special handle another IE7 muddy hack -> extra
		            // padding-bottom that is added to table to prevent non-necesarry
		            // vertical scrollers
		            if(elem.scrollWidth > elem.offsetWidth) {
			            $(elem).height($(elem).height() - 20);
			            // show hidden vertical scroller if it is again needed after
			            // height is decreased.
            ...
            /**
			 * This adds a "resize" listener to the window, and every window "resize" it will call a method to
			 * recalculate the height of a single div (flexid) based on the position of:
			 * <ul>
			 * <li>The 'bottom' position of the element just above it (get it's top position and add it's height)</li>
			 * <li>A bottom 'offset', currently an integer in pixels. It will become a "bottom item" id later, so we
			 * can auto-size a div "sandwiched" between two other elements.</li>
			 * </ul>
            ...
            debug : function(debugId, posX, posY, debugInfoHtml) {
	            // Be aware that debugId must not start with digit when using FF! Just
	            // lost 1 hour to learn this...
            ...
            blockUI : function() {
	            ...
	            // -- Create a backdrop div sized 100% overlaying the body, initially
	            // just fully transparant (just blocking mouseclicks).
            ...
            printtext : function(id) {
	            ...
	            if(textData) {
		            try {
			            // Create embedded sizeless div to contain the iframe,
			            // invisibly.
			            var div = document.getElementById('domuiprif');
			            if(div)
				            div.innerHTML = "";
			            ...
			            // -- Create an iframe loading the required thingy.
			            var frmname = "dmuifrm" + (WebUI._frmIdCounter++); // Create
			            // unique
			            // name to
			            // circumvent
			            // ffox
			            ...
			            $(div).html('<iframe id="' + frmname + '" name="' + frmname + '" width="1000px" height="1000px"/>'); // well,
			            // this
			            // is
			            // simple
			            // text
            ...
            handleDevelopmentMode : function() {
	            ...
	            $(document.body).bind("mousemove", function(e) {
		            ...
		            WebUI._debugMouseTarget = e.target; // e.srcElement ||
		            // e.originalTarget;
	            });
            },
            ...
            /**
			 * Register the popup. If the mouse leaves the popup window the popup needs to send a POPINCLOSE? command;
			 * this will tell DomUI server that the popin needs to go. If an item inside the popin is clicked it should
			 * mean the popin closes too; at that point we will deregister the mouse listener to prevent sending double
			 * events.
			 * 
			 * @param id
			 */
            registerPopinClose : function(id) {
	            ...
	            // $(document.body).bind("beforeclick", WebUI.popinBeforeClick); //
	            // Called when a click is done somewhere - not needed anymore, handled
	            // from java
            },
            ...
            // We need to re-show element to force IE7 browser to recalculate correct
            // height of element. This must be done to fix some IE7 missbehaviors.
            refreshElement : function(id) {
	            var elem = document.getElementById(id);
	            if(elem) {
		            $(elem).hide();
		            $(elem).show(1); // needs to be done on timeout/animation,
		            // otherwise it still fails to recalculate...
	            }
            },

            // Use this to make sure that item would be visible inside parent scrollable
            // area. It uses scroll animation. In case when item is already in visible
            // part, we just do single blink to gets user attention ;)
            scrollMeToTop : function(elemId, selColor, offset) {
	            ...
	            if(parent.scrollHeight > parent.offsetHeight) { // if parent has
		            // scroll
		            ...
		            if(elemPos > 0 && elemPos < parent.offsetHeight) {
			            ...
			            if($.browser.msie && $.browser.version.substring(0, 1) == "8") {
				            if($(elem).height() == 0) {
					            newPos = newPos - 15; // On IE8 we need this
					            // correction :¬|
				            }
			            }
			            if(offset) {
            ...
            // Use this to make sure that option in dropdown would be visible. It needs
            // fix only in FF sinve IE would always make visible selected option.
            ...
            // Returns T if browser is really using IE7 rendering engine (since IE8
            // compatibility mode presents browser as version 7 but renders as IE8!)
            isReallyIE7 : function() {
	            // Stupid IE8 in compatibility mode lies that it is IE7, and renders as
	            // IE8! At least we can detect that using document.documentMode (it is 8
	            // in that case)
	            // document.documentMode == 7 --- IE8 running in IE7 mode
	            // document.documentMode == 8 --- IE8 running in IE8 mode or IE7
	            // Compatibility mode
            ...
            // Returns T if browser is IE of at least version 9 and does not run in any
            // of compatibility modes for earlier versions
            isNormalIE9plus : function() {
	            return ($.browser.msie && parseInt($.browser.version) >= 9 && document.documentMode >= 9);
            },

            // Returns T if browser is IE of at least version 8 even if it runs in IE7
            // compatibility mode
            ...
            initAutocomplete : function(inputId, selectId) {
	            ...
	            $(select).keypress(function(event) {
		            // esc hides select and prevents fireing of click and blur handlers
		            // that are temporary disconnected while focus moves back to input
		            var keyCode = WebUI.normalizeKey(event);
...
/**
 * This handles ROW mode drops. It locates the nearest row in the TBody for this dropTarget and decides to put the
 * thingy BEFORE or AFTER that row. The boundary there then gets highlighted.
 */
WebUI._ROW_DROPZONE_HANDLER = {
    locateBest : function(dz) {
	    ...
	    for( var i = 0; i < tbody.childNodes.length; i++) {
		    ...
		    if(position) {
			    // console.debug('mouse:' +mousePos+','+mouseX+' row:
			    // prevPosition.top='+prevPosition.top+",
			    // position.top="+position.top+", index="+position.index);

			    // -- Is the mouse IN the Y range for this row?
			    if(mousePos >= prevPosition.top && mousePos < position.top) {
				    // -- Cursor is WITHIN this node. Is it near the TOP or near
				    // the
				    // BOTTOM?
				    gravity = 0;
				    ...
				    // console.debug('ACCEPTED top='+prevPosition.top+',
				    // bottom='+position.top+', hy='+hy+',
				    // rowindex='+(rowindex-1));
				    // console.debug('index='+prevPosition.index+',
				    // gravety='+gravity);
			    ...
			    // row).
			    // //MVE
			    // console.debug('ACCEPTED BEFORE node by='+prevPosition.top+',
			    // ey='+position.top+', rowindex='+rowindex-1);
			    // return {
			    // index :rowindex,
			    ...
			    // };
			    // }
			    // console.debug('REFUSED by='+prevPosition.top+",
			    // ey="+position.top+", rowindex="+rowindex);
		    } else {
    ...
    getColIndex : function(tr, mouseX) {
	    ...
	    for(j = 0; j < tr.childNodes.length; j++) {
		    ...
		    if(mouseX >= left && mouseX < right) {
			    // because only the left position can be asked, the check is
			    // done for the previous collumn
			    return j - 1;
		    }
    ...
    checkRerender : function(dz) {
	    ...
	    // console.debug("checkRerender: "+b.iindex+", "+b.index+",
	    // g="+b.gravity);
	...
	ctl.bind('fileQueueError', function(event, file, errorCode, message) {
		try {
			if(errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
				...
				// (message === 0 ? "You have reached the upload limit." : "You
				// may select " + (message > 1 ? "up to " + message + " files."
				// : "one file.")));
				return;
			}
			var uf = new WebUI.UploadFile(file, target);
...
WebUI.UploadFile = function(file, target, cancelFn) {
	...
	if(this._ui.length == 0) {
		// -- Create the UI.
		target.append("<div id='" + this._id
		    + "' class='ui-bupl-file'><div class='ui-bupl-inner ui-bupl-pending'><a href='#' class='ui-bupl-cancl'> </a><div class='ui-bupl-name'>"
		    + file.name + "</div><div class='ui-bupl-stat'>" + DomUI._T.buplPending + "</div><div class='ui-bupl-perc'></div></div></div>");
		this._ui = $('#' + file.id);
		if(cancelFn) {
			var me = this;
...
WebUI.UploadFile.prototype.uploadComplete = function() {
	...
	$(".ui-bupl-inner", this._ui).removeClass("ui-bupl-pending").removeClass("ui-bupl-running").removeClass("ui-bupl-error").addClass(
	    "ui-bupl-complete");
