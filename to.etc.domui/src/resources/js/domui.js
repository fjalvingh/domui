...
var WebUI = {
    /**
	 * can be set to true from server code with appendJavaScript so that the expired messages will not show and block
	 * effortless refresh on class reload. Configurable in .developer.properties domui.hide-expired-alert.
	 */
    ...
    /**
	 * Handle enter key pressed on keyPress for component with onLookupTyping listener. This needs to be executed on
	 * keyPress (was part of keyUp handling), otherwise other global return key listener (returnKeyPress handler) would
	 * fire.
	 */
    ...
    /*
	 * Executed as onkeyup event on input field that has implemented listener for onLookupTyping event. In case of
	 * return key call lookupTypingDone ajax that is transformed into onLookupTyping(done=true). In case of other key,
	 * lookupTyping funcion is called with delay of 500ms. Previuosly scheduled lookupTyping function is canceled. This
	 * cause that fast typing would not trigger ajax for each key stroke, only when user stops typing for 500ms ajax
	 * would be called by lookupTyping function.
	 */
    scheduleOnLookupTypingEvent : function(id, event) {
	    ...
	    if(isDownArrowKey || isUpArrowKey) {
		    ...
		    if(trNodes.length > 0) {
			    ...
			    trNode = $(node.parentNode).children("div.ui-lui-keyword-popup").children("div").children("table").children("tbody").children(
			        "tr:nth-child(" + selectedIndex + ")").get(0);
			    if(trNode) {
    ...
    /*
	 * In case of longer waiting for lookupTyping ajax response show waiting animated marker. Function is called with
	 * delay of 500ms from ajax.beforeSend method for lookupTyping event.
	 */
    ...
    /*
	 * Hiding waiting animated marker that was shown in case of longer waiting for lookupTyping ajax response. Function
	 * is called from ajax.completed method for lookupTyping event.
	 */
    ...
    handleErrorAsy : function(request, status, exc) {
	    ...
	    /*
		 * As usual there is a problem with error reporting: if the request is aborted because the browser reloads the
		 * page any pending request is cancelled and comes in here- but with the wrong error code of course. So to
		 * prevent us from showing an error message: set a timer to show that message 250 milli's later, and hope the
		 * stupid browser disables that timer.
		 */
	    setTimeout(function() {
    ...
    /*
	 * IE/FF compatibility: IE only has the 'keycode' field, and it always hides all non-input like arrows, fn keys etc.
	 * FF has keycode which is used ONLY for non-input keys and charcode for input.
	 */
    ...
    /** ******************* Scheduler component ************************ */
    /**
	 * This handles item placement for a single calendar. It "discovers" the calendar divs using fixed css class names,
	 * and uses "extra" attributes generated on the root divs for the parts to get the calendar data. This handles
	 * overlapping appointments by dividing a full day lane into multiple "ways". A day lane starts with one "way". For
	 * every appointment added to a day we check if this appointment overlaps another appointment on this way[0]. If it
	 * does we create a next way (or use it if it already exists) and check if the appointment clashes there. We do this
	 * until we find a way that can accept the appointment without overlaps. We do this for all appointments. At the end
	 * we have a list of ways for every day. We now start rendering the appointments by going through them per way, then
	 * per appointment. For every appointment on a way we check if there is an appointment on a LATER way that overlaps.
	 * If not the width of the appointment includes all LATER ways that do not overlap. For this code to work no item
	 * may "overlap" a day. The server code takes care of splitting long appointments into multiple "items" here.
	 */
    ...
    /**
	 * None of the "standard" JS libraries like Rico or Prototype have code that actually <i>works</i> to get the
	 * actual <i>page or absolute</i> position of elements when scrolling is used. All of them unconditionally add
	 * scroll offsets to the relative positions but scrolling *will* cause items to become *invisible* because they are
	 * scrolled out of view. The calls here obtain a location for elements taking scrolling into account, and they will
	 * return null if the item is not visible at all.
	 */
    ...
    /**
	 * Send Ajax request to the server every 2 minutes. This keeps the session alive. The response can contain commands
	 * to execute which will indicate important events or changes have taken place.
	 */
    ...
    /**
	 * Called for ie upload garbage only, this tries to decode the utter devastating mess that ie makes from xml uploads
	 * into an iframe in ie8+. Sigh. The main problem with IE is that the idiots that built it mess up XML documents
	 * sent to it: when the iframe we use receives an xml document the idiot translates it into an html document- with
	 * formatted tags and minus signs before it to collapse them. Instead of just /rendering/ that they actually
	 * /generate/ that as the content document of the iframe- so getting that means you get no XML at all. This
	 * abomination was more or less fixed in ie9 - but of course they fucked up compatibility mode badly. Everything
	 * below ie9 ==================== Everything below ie9 has a special property "XMLDocument" attached to the
	 * "document" property of the window. This property contains the original XML that was used as an XML tree. So for
	 * these browsers we just get that and move on. IE9 in native mode ================== IE9 in native mode does not
	 * mess up the iframe content document, so this mode takes the path of all browsers worth the name. Since the
	 * original problem was solved the XMLDocument property no longer exists. IE9 in compatibility mode (IE7)
	 * =============================== This gets funny. In this mode the browser /still/ messes up the iframe's content
	 * model like the old IE's it emulates. But of course the XMLDocument property is helpfully removed - EVEN IN THIS
	 * MODE. Remember: this mode is also entered if you are part of a frameset or iframe that is part of an mode: the
	 * topmost frameset/page determines the mode for the entire set of pages - they are that stupid. I know no other
	 * "workaround" than the horrible concoction below; please turn away if you have a weak stomach.... In this case we
	 * get the "innerText" content of the iframe. This differers from innerHTML in that all html tags that IE added are
	 * removed, and only the text is retained. Since the html was generated by IE in such a way that the xml was
	 * presented to the user - this is actually most of our XML... But it contains some problems: - there is extra
	 * whitespace around it's edges, so we need to remove that.. - All tags start on a new line with a '-' sign before
	 * them... Remove all that... - The result will have the &amp; entity replaced by & because they are really stupid,
	 * again. So replace that too. The resulting thing can sometimes be parsed as XML and then processing continues. But
	 * it is far from perfect. The biggest problem is that the resulting xml has not properly reserved the original
	 * whitespace; this may lead to rendering problems. But as far as I know no other solution to this stupifying bug is
	 * possible. Please prove me wrong.
	 * 
	 * @param e
	 */
    ...
    updateUpload : function(doc, ifr) {
	    ...
	    /*
		 * 20081015 jal De iframe zou verwijderd moeten worden, maar als ik dat doe onder Firefox dan blijft de browser
		 * in "busy" mode staan, met een rode enabled "stop" knop en een "busy" mouse.. Voor nu hergebruiken we de
		 * bestaande iframe dan maar en accepteren de verloren resources.
		 */
    ...
    /**
	 * Called at page unload time, this quickly tries to send an obituary to the server. This is currently unconditional
	 * but can later be augmented to send the obituary only when the browser window closes.
	 */
    ...
    /**
	 * When mouse is downed on an item that is draggable. This moves to PREDRAG mode where the first move will create a
	 * visible representation of this node, ready for dropping.
	 */
    ...
    /**
	 * Resets the dropzone timer. Called when in DRAGGING mode and the mouse moves, this resets any "open" dropzone
	 * indicators and resets the timer on which drop zone effects are done. This causes the dropzone indicator delay
    ...
    /**
	 * Fires when in DRAGGING mode and the mouse has not moved for a while. It initiates the rendering of any drop zone
	 * indicators if the mouse is above a drop zone.
	 */
    ...
    /**
	 * Gets or recalculates the list of possible drop targets and their absolute on-screen position. This list is used
	 * to determine if the mouse is "in" a drop target. The list gets cached globally in the WebUI object; if an AJAX
	 * request is done the list gets cleared.
	 */
    ...
    /**
	 * Register the popup. If the mouse leaves the popup window the popup needs to send a POPINCLOSE? command; this will
	 * tell DomUI server that the popin needs to go. If an item inside the popin is clicked it should mean the popin
	 * closes too; at that point we will deregister the mouse listener to prevent sending double events.
	 * 
	 * @param id
	 */
