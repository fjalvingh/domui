...
(function($) {
	...
	function executeXML(xml) {
		...
		if(rname == 'redirect') {
			...
			return true; // do nothing actually, page is in process of redirecting to some other page and we need to
...
// try {
		...
		function process(commands) {
			...
			for( var i = 0; i < commands.length; i++) {
				...
				if(!q) {
					// -- Node sans select-> we are in trouble -> this is probably a server error/response. Report
					// session error, then reload. (Marc, 20111017)
					...
					try {
						...
						for( var ai = 0, attr = ''; ai < src.attributes.length; ai++) {
							...
							if(n == 'style') { // IE workaround
								...
								// We need this dirty fix for IE7 to force height recalculation of divs that has just
								// become visible (IE7 sometimes fails to calculate height that stays 0!).
								...
								// -- jal 20100720 handle disabled, readonly, checked differently: these are either
								// present or not present; their value is always the same.
// alert('changeAttr: id='+dest.id+' change '+n+" to "+v);
								...
								if(dest.tagName.toLowerCase() == 'select' && n == 'class' && $.browser.mozilla) {
									...
									dest.selectedIndex = 1; // jal 20100720 Fixes problem where setting BG color on
															// select removes the dropdown button image
...
// }
					...
					if(inline) {
						// -- 20091110 jal When inlining we are in trouble if domjs_ is used... The domjs_ mechanism is
						// replaced with setDelayedAttributes in java.
						if(n.substring(0, 6) == 'domjs_') {
							alert('Unsupported domjs_ attribute in INLINE mode: ' + n);
						} else {
							// -- jal 20100720 handle disabled, readonly, checked differently: these are either present
							// or not present; their value is always the same.
...
// this._xxxw = true;
						...
						// -- jal 20100720 handle disabled, readonly, checked differently: these are either present or
...
var WebUI = {
    ...
    getInputFields : function(fields) {
	    ...
	    for( var i = q1.length; --i >= 0;) {
		    ...
		    if(t.type == 'file')
			    continue;
		    if(t.type == 'hidden' && !t.getAttribute('s')) // All hidden input nodes are created directly in browser
															// java-script and because that are filtered out from server
															// requests.
			    continue;
...
// } else
    ...
    valuechanged : function(h, id) {
	    // FIXME 20100315 jal Temporary fix for bug 680: if a DateInput has a value changed listener the onblur does not
		// execute. So handle it here too.... The fix is horrible and needs generalization.
    ...
    hideLookupTypingPopup : function(id) {
	    ...
	    if($.browser.msie) {
		    // IE kills event stack (click is canceled) when z index is set during onblur event handler... So, we need
    ...
    showLookupTypingPopupIfStillFocusedAndFixZIndex : function(id) {
	    ...
	    if(qDivPopup.length > 0) {
		    ...
		    // z-index correction must be set manually from javascript (because some bug in IE7 -> if set from domui
			// renders incorrectly until page is refreshed?)
	    ...
	    if(trNods && trNods.length > 0) {
		    for( var i = 0; i < trNods.length; i++) {
			    ...
			    // we need this jquery way of attaching events, if we use trNod.setAttribute("onmouseover",...) it does
				// not work in IE7
    ...
    handleError : function(request, status, exc) {
	    ...
	    if(txt.length == 0) {
		    // -- Firefox fix: if the page is unloading but a request is pending this may cause an status=ERROR for that
			// page. Prevent us from then overwriting the new document....
		    if(status == "error")
			    return;
...
// alert('Node is '+arguments[0]);
    ...
    showCalendar : function(id, withtime) {
	    ...
	    var params = {
	        ...
	        showsTime : withtime,
	        timeFormat : "24",
	        electric : false, // jal 20110125 Fixes bug 885- do not update the field when moving to prevent firing the
								// change handler.
	        step : 2,
	        position : null,
	        cache : false
	    ...
	    var cal = new Calendar(null, params.date, WebUI.onDateSelect, function(cal) {
		    cal.hide();
		    cal.destroy();
	    });
    ...
    fileUploadChange : function(e) {
	    ...
	    if(!iframe) {
		    if(jQuery.browser.msie && !WebUI.isNormalIE9plus()) { // MicroMorons report ie9 for ie7 emulation of
																	// course
    ...
    ieUpdateUpload : function(e) { // Piece of crap
	    ...
	    if(iframe.contentWindow && iframe.contentWindow.document.XMLDocument) {
		    xml = iframe.contentWindow.document.XMLDocument; // IMPORTANT Fucking MS Crap!!!! See
...
// }
    ...
    scrollRight : function(bRight) {
	    ...
	    var tabsTotalWidth = $('li:last', scrlNavig).width() + 8 /* paddong = 8 */+ $('li:last', scrlNavig).offset().left
    ...
    recalculateScrollers : function(scrlNavigId) {
	    ...
	    var tabsTotalWidth = $('li:last', scrlNavig).width() + 8 /* paddong = 8 */+ $('li:last', scrlNavig).offset().left
	        - $('li:first', scrlNavig).offset().left;
	    var tabsVisibleWidth = $(scrlNavig).width() - 2 * $('.ui-stab-scrl-right', scrlNavig).width();
	    // WebUI.debug('debug1', 50, 150, 'width:' + ($('li:last',scrlNavig).width() + 8 +
		// $('li:last',scrlNavig).offset().left - $('li:first',scrlNavig).offset().left));
	    // WebUI.debug('debug2', 50, 200, 'offsetX:' + ($('li:first',scrlNavig).offset().left - $('ul',
    ...
    stretchHeight : function(elemId) {
	    ...
	    $(elem).siblings().each(
	        function(index, node) {
		        // do not count target element and other siblings positioned absolute or relative to parent in order to
				// calculate how much space is actually taken / available
		        if(node != elem && $(node).css('position') == 'static'
		            && ($(node).css('float') == 'none' || $(node).css('width') != '100%' /*
																							 * count in floaters that
																							 * occupies total width
																							 */)) {
	    ...
	    var elemDeltaHeight = $(elem).outerHeight(true) - $(elem).height(); // we need to also take into account elem
																			// paddings, borders... So we take its delta
																			// between outter and inner height.
	    ...
	    if($.browser.msie && $.browser.version.substring(0, 1) == "7") {
		    // we need to special handle another IE7 muddy hack -> extra padding-bottom that is added to table to
			// prevent non-necesarry vertical scrollers
...
// console.debug('block, busy=', WebUI._busyCount);
	    ...
	    // -- Create a backdrop div sized 100% overlaying the body, initially just fully transparant (just blocking
		// mouseclicks).
...
// console.debug('unblock, busy=', WebUI._busyCount);
    ...
    documentPrintIE : function(url) {
	    try {
		    ...
		    var frmname = "dmuifrm" + (WebUI._frmIdCounter++); // Create unique name to circumvent ffox "print only
																// once" bug
...
// $(document.body).unbind("mousedown", WebUI.popinMouseClose);
    ...
    // We need to re-show element to force IE7 browser to recalculate correct height of element. This must be done to
	// fix some IE7 missbehaviors.
    ...
    // Use this to make sure that item would be visible inside parent scrollable area. It uses scroll animation. In case
	// when item is already in visible part, we just do single blink to gets user attention ;)
    ...
    // Use this to make sure that option in dropdown would be visible. It needs fix only in FF sinve IE would always
	// make visible selected option.
    ...
    // Returns T if browser is really using IE7 rendering engine (since IE8 compatibility mode presents browser as
	// version 7 but renders as IE8!)
    isReallyIE7 : function() {
	    // Stupid IE8 in compatibility mode lies that it is IE7, and renders as IE8! At least we can detect that using
		// document.documentMode (it is 8 in that case)
    ...
    // Returns T if browser is IE of at least version 9 and does not run in any of compatibility modes for earlier
	// versions
    ...
    // connects input to usually hidden list select and provides autocomplete feature inside input. Down arrow does show
	// and focus select list.
    initAutocomplete : function(inputId, selectId) {
	    ...
	    $(select).keypress(function(event) {
		    // esc hides select and prevents fireing of click and blur handlers that are temporary disconnected while
			// focus moves back to input
		    var keyCode = WebUI.normalizeKey(event);
...
WebUI._ROW_DROPZONE_HANDLER = {
    locateBest : function(dz) {
	    ...
	    for( var i = 0; i < tbody.childNodes.length; i++) {
		    ...
		    if(position) {
			    // console.debug('mouse:' +mousePos+','+mouseX+' row: prevPosition.top='+prevPosition.top+",
			    ...
			    if(mousePos >= prevPosition.top && mousePos < position.top) {
				    ...
				    // console.debug('ACCEPTED top='+prevPosition.top+', bottom='+position.top+', hy='+hy+',
					// rowindex='+(rowindex-1));
			    ...
			    // //MVE
			    // console.debug('ACCEPTED BEFORE node by='+prevPosition.top+', ey='+position.top+',
				// rowindex='+rowindex-1);
			    // return {
			    // index :rowindex,
