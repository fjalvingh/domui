...
public class DutchStemmer implements IStemmer {
	/**
	 * Buffer for the terms while stemming them.
	 */
	private StringBuffer m_sb = new StringBuffer();
	private boolean m_removedE;
	private Map<String, Object> m_stemDict;
	...
	public String stem(String term) {
		...
		if(!isStemmable(term))
			return term;
		if(m_stemDict != null && m_stemDict.containsKey(term))
			if(m_stemDict.get(term) instanceof String)
				return (String) m_stemDict.get(term);
			else
				return null;
		...
		substitute(m_sb);
		storeYandI(m_sb);
		m_R1 = getRIndex(m_sb, 0);
		m_R1 = Math.max(3, m_R1);
		step1(m_sb);
		step2(m_sb);
		m_R2 = getRIndex(m_sb, m_R1);
		step3a(m_sb);
		step3b(m_sb);
		step4(m_sb);
		reStoreYandI(m_sb);
	...
	private boolean enEnding(StringBuffer sb) {
		...
		for(int i = 0; i < enend.length; i++) {
			...
			if(s.endsWith(end) && index >= m_R1 && isValidEnEnding(sb, index - 1)) {
	...
	private void step1(StringBuffer sb) {
		...
		String s = sb.toString();
		int lengthR1 = sb.length() - m_R1;
		int index;

		if(s.endsWith("heden")) {
			sb.replace(m_R1, lengthR1 + m_R1, sb.substring(m_R1, lengthR1 + m_R1).replaceAll("heden", "heid"));
			return;
		}
		...
		if(s.endsWith("se") && (index = s.length() - 2) >= m_R1 && isValidSEnding(sb, index - 1)) {
			sb.delete(index, index + 2);
			return;
		}
		if(s.endsWith("s") && (index = s.length() - 1) >= m_R1 && isValidSEnding(sb, index - 1)) {
	...
	private void step2(StringBuffer sb) {
		...
		if(index >= m_R1 && s.endsWith("e") && !isVowel(sb.charAt(index - 1))) {
			sb.delete(index, index + 1);
			unDouble(sb);
			m_removedE = true;
		}
	...
	private void step3a(StringBuffer sb) {
		...
		if(s.endsWith("heid") && index >= m_R2 && sb.charAt(index - 1) != 'c') {
	...
	private void step3b(StringBuffer sb) {
		...
		if((s.endsWith("end") || s.endsWith("ing")) && (index = s.length() - 3) >= m_R2) {
			sb.delete(index, index + 3);
			if(sb.charAt(index - 2) == 'i' && sb.charAt(index - 1) == 'g') {
				if(sb.charAt(index - 3) != 'e' & index - 2 >= m_R2) {
					index -= 2;
					sb.delete(index, index + 2);
				}
			}
		...
		if(s.endsWith("ig") && (index = s.length() - 2) >= m_R2) {
			if(sb.charAt(index - 1) != 'e')
				sb.delete(index, index + 2);
			return;
		}
		if(s.endsWith("lijk") && (index = s.length() - 4) >= m_R2) {
			sb.delete(index, index + 4);
			step2(sb);
			return;
		}
		if(s.endsWith("baar") && (index = s.length() - 4) >= m_R2) {
			sb.delete(index, index + 4);
			return;
		}
		if(s.endsWith("bar") && (index = s.length() - 3) >= m_R2) {
			if(m_removedE)
				sb.delete(index, index + 3);
			return;
		}
	...
	void setStemDictionary(Map<String, Object> dict) {
		m_stemDict = dict;
	}
