...
//			checkDomuiMetadata();
	 ...
	 * @param g
	 */
	static private void checkBooleanMapping(Method g) {
		if(Boolean.class.isAssignableFrom(g.getReturnType())) {
			Transient tr = g.getAnnotation(Transient.class);
			if(null == tr) {
				problem(Severity.WARNING, "Do not use Boolean wrappers. Use boolean as $DEITY intended.");
			} else {
	 ...
	 * @param g
	 */
	static private void checkOneToMany(Method g) {
	 ...
	 * @param g
	 */
	static private void checkDateMapping(Method g) {
	 ...
	 * @param g
	 */
	static private void checkEnumMapping(Method g) {
	...
	static private void checkDomuiMetadata() {
		try {
			m_currentProperty = null;
