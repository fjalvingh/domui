...
public class ServiceMethodDefinition {
	private ServiceClassDefinition	m_ServiceClassDefinition;
	...
	ServiceMethodDefinition(ServiceClassDefinition hi, String name) {
		m_name = name;
	...
	private Method findMethod(String name) throws Exception {
		...
		for(Method m : cl.getMethods()) {
			if(m.getName().equals(name)) {
				if(foundm != null)
					throw new UnknownServiceMethodException(m_ServiceClassDefinition.getHandlerClass(), m_name, "The method '" + name + "' occurs 2ce [" + foundm.toGenericString() + " and "
						+ m.toGenericString() + "]");
				foundm = m;
			}
		}
