...
public class Janitor implements Runnable {
	...
	private Vector					m_task_v				= new Vector(20);
	...
	private synchronized void insertOrdered(JanitorTask jt) {
		...
		{
			JanitorTask tjt = (JanitorTask) m_task_v.elementAt(i);
			if(jt.m_t_next < tjt.m_t_next) {
				//-- New one has to execure before current: insert!!
				m_task_v.insertElementAt(jt, i); // Insert before CURRENT
				return;
			}
		}
		...
		try {
			...
			if(!handleJobQueue()) // Check all finished stuff in the job queue.
			{
				...
				if(m_t_freeslot - getSchedTime() > 10 * 60 * 1000) {
					Panicker.logUnexpected("Janitor: No jobs have completed in 10 minutes; cannot schedule!!");
	...
	private JanitorTask getRunnableTask() {
		...
		{
			for(int i = 0; i < m_task_v.size(); i++) // For all elements,
			{
				JanitorTask jt = (JanitorTask) m_task_v.elementAt(i);
