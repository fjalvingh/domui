...
public class XmlParser
{
	...
	private synchronized void doParse(String systemId, String publicId, Reader reader, InputStream stream, String encoding)
			throws java.lang.Exception
	{
		m_basePublicId = publicId;
		m_baseURI = systemId;
		m_baseReader = reader;
		m_baseInputStream = stream;
		...
		pushURL("[document]", m_basePublicId, m_baseURI, m_baseReader, m_baseInputStream, encoding);
	...
	void error(String message, String textFound, String textExpected) throws java.lang.Exception
	{
		...
		if(m_handler != null)
		{
			...
			{
				uri = m_externalEntity.getURL().toString();
			}
			m_handler.error(message, uri, m_line, m_column);
		}
	}
	...
	void parsePI() throws java.lang.Exception
	{
		...
		if(m_handler != null)
		{
			m_handler.processingInstruction(name, dataBufferToString());
		}
	}
	...
	void checkEncoding(String encodingName, boolean ignoreEncoding) throws java.lang.Exception
	{
		...
		switch(m_encoding)
		{
			...
			case ENCODING_UTF_8:
				...
				{
					m_encoding = ENCODING_ISO_8859_1;
				}
	...
	void parseDoctypedecl() throws java.lang.Exception
	{
		...
		if(m_handler != null)
		{
			m_handler.doctypeDecl(doctypeName, ids[0], ids[1]);
		}
	...
	void parseElement() throws java.lang.Exception
	{
		String gi;
		char c;
		int oldElementContent = m_currentElementContent;
		String oldElement = m_currentElement;
		...
		m_currentElement = gi;
		m_currentElementContent = getElementContentType(gi);
		if(m_currentElementContent == CONTENT_UNDECLARED)
		{
			m_currentElementContent = CONTENT_ANY;
		}
		...
		if(atts != null)
		{
			...
			loop : while(atts.hasMoreElements())
			{
				...
				for(int i = 0; i < m_tagAttributePos; i++)
				{
					if(m_tagAttributes[i] == aname)
					{
						continue loop;
					}
				}
				...
				if(m_handler != null)
				{
					m_handler.attribute(aname, getAttributeExpandedValue(gi, aname), false);
				}
			}
		}
		...
		switch(c)
		{
			...
			case '/':
				...
				{
					m_handler.startElement(gi);
					m_handler.endElement(gi);
				}
				break;
		}

		// Restore the previous state.
		m_currentElement = oldElement;
		m_currentElementContent = oldElementContent;
	}
	...
	void parseAttribute(String name) throws java.lang.Exception
	{
		...
		if(m_handler != null)
		{
			m_handler.attribute(aname, value, true);
		}
		m_dataBufferPos = 0;

		// Note that the attribute has been
		// specified.
		if(m_tagAttributePos == m_tagAttributes.length)
		{
			String newAttrib[] = new String[m_tagAttributes.length * 2];
			System.arraycopy(m_tagAttributes, 0, newAttrib, 0, m_tagAttributePos);
			m_tagAttributes = newAttrib;
		}
		m_tagAttributes[m_tagAttributePos++] = aname;
	}
	...
	void parseETag() throws java.lang.Exception
	{
		String name;
		name = readNmtoken(true);
		if(name != m_currentElement)
		{
			error("mismatched end tag", name, m_currentElement);
		}
		skipWhitespace();
		require('>');
		if(m_handler != null)
		{
			m_handler.endElement(name);
		}
	}
	...
	void parseContent() throws java.lang.Exception
	{
		...
		while(true)
		{
			...
			switch(m_currentElementContent)
			{
				case CONTENT_ANY:
				case CONTENT_MIXED:
					parsePCData();
	...
	void parseDefault(String elementName, String name, int type, String enumeration) throws java.lang.Exception
	{
		...
		if(tryRead('#'))
		{
			if(tryRead("FIXED"))
			{
				...
				requireWhitespace();
				m_context = CONTEXT_ATTRIBUTEVALUE;
			...
			for(int i = m_rbpos; i < m_rblen; i++)
			{
				switch(m_readBuffer[i])
				{
					...
					case '<':
						...
						{
							m_line += lineAugment;
							m_column = columnAugment;
						}
						else
						{
							m_column += columnAugment;
						}
						dataBufferAppend(m_readBuffer, start, i - start);
						return;
					default:
						columnAugment++;
				}
	...
	void skipWhitespace() throws java.lang.Exception
	{
		...
		if(USE_CHEATS)
		{
			...
			loop : for(int i = m_rbpos; i < m_rblen; i++)
			{
				switch(m_readBuffer[i])
				{
					case ' ':
					...
					case '\n':
						...
						break;
					case '%':
						if(m_context == CONTEXT_DTD || m_context == CONTEXT_ENTITYVALUE)
						{
							break loop;
						} // else fall through...
					default:
	...
	String readNmtoken(boolean isName) throws java.lang.Exception
	{
			...
			loop : for(int i = m_rbpos; i < m_rblen; i++)
			{
				switch(m_readBuffer[i])
				{
					...
					case '#':
						...
						if(i == start)
						{
							error("name expected", m_readBuffer[i], null);
						}
						m_rbpos = i;
						return intern(m_readBuffer, start, i - start);
				}
			}
		}
		...
		while(true)
		{
			...
			switch(c)
			{
				...
				case '/':
					unread(c);
					if(m_nameBufferPos == 0)
					{
						error("name expected", null, null);
					}
					String s = intern(m_nameBuffer, 0, m_nameBufferPos);
					m_nameBufferPos = 0;
					return s;
				default:
					m_nameBuffer = (char[]) extendArray(m_nameBuffer, m_nameBuffer.length, m_nameBufferPos);
					m_nameBuffer[m_nameBufferPos++] = c;
			}
		}
	}
	...
	void dataBufferAppend(char c)
	{
		...
		m_dataBuffer = (char[]) extendArray(m_dataBuffer, m_dataBuffer.length, m_dataBufferPos);
		m_dataBuffer[m_dataBufferPos++] = c;
	}
	...
	{
		m_dataBuffer = (char[]) extendArray(m_dataBuffer, m_dataBuffer.length, m_dataBufferPos + length);
		System.arraycopy(ch, start, m_dataBuffer, m_dataBufferPos, length);
		m_dataBufferPos += length;
	}
	...
	void dataBufferNormalize()
	{
		int i = 0;
		int j = 0;
		int end = m_dataBufferPos;

		// Skip whitespace at the start.
		while(j < end && isWhitespace(m_dataBuffer[j]))
		{
			j++;
		}

		// Skip whitespace at the end.
		while(end > j && isWhitespace(m_dataBuffer[end - 1]))
		{
			end--;
		}
		...
		while(j < end)
		{
			...
			if(isWhitespace(c))
			{
				...
				{}
				m_dataBuffer[i++] = ' ';
				m_dataBuffer[i++] = m_dataBuffer[j - 1];
			}
			else
			{
				m_dataBuffer[i++] = c;
	...
	{
		String s = new String(m_dataBuffer, 0, m_dataBufferPos);
		m_dataBufferPos = 0;
		return s;
	}
		...
		if(m_dataBufferPos > 0)
		{
			switch(m_currentElementContent)
			{
				...
				case CONTENT_ANY:
					if(m_handler != null)
					{
						m_handler.charData(m_dataBuffer, 0, m_dataBufferPos);
					}
					break;
				case CONTENT_ELEMENTS:
					if(m_handler != null)
					{
						m_handler.ignorableWhitespace(m_dataBuffer, 0, m_dataBufferPos);
					}
					break;
			}
			m_dataBufferPos = 0;
	...
	public String intern(char ch[], int start, int length)
	{
		...
		// Get the bucket.
		Object bucket[] = (Object[]) m_symbolTable[hash];
		if(bucket == null)
		{
			m_symbolTable[hash] = bucket = new Object[8];
		}
	...
	{
		Object element[] = (Object[]) m_elementInfo.get(name);
		if(element == null)
		{
	...
	void setElement(String name, int contentType, String contentModel, Hashtable attributes) throws java.lang.Exception
	{
		...
		element = (Object[]) m_elementInfo.get(name);
		...
		m_elementInfo.put(name, element);
	}
	...
	{
		Object entity[] = (Object[]) m_entityInfo.get(ename);
		if(entity == null)
		{
	...
	{
		Object notation[] = (Object[]) m_notationInfo.get(nname);
		if(notation == null)
		{
			return null;
		}
	...
	void setNotation(String nname, String pubid, String sysid) throws java.lang.Exception
	{
		...
		if(m_notationInfo.get(nname) == null)
		{
			notation = new Object[2];
			notation[0] = pubid;
			notation[1] = sysid;
			m_notationInfo.put(nname, notation);
		}
		else
		{
	...
	char readCh() throws java.lang.Exception
	{
		...
		// declarations.
		if(c == '%' && (m_context == CONTEXT_DTD || m_context == CONTEXT_ENTITYVALUE))
		{
			char c2 = readCh();
			unread(c2);
			if(!isWhitespace(c2))
			{
				parsePEReference(m_context == CONTEXT_ENTITYVALUE);
				return readCh();
			}
		}
	...
	void pushURL(String ename, String publicId, String systemId, Reader reader, InputStream stream, String encoding) throws java.lang.Exception
	{
		...
		m_readBuffer = new char[READ_BUFFER_MAX + 4];
		m_rbpos = 0;
		m_rblen = 0;
		m_rboverflow = -1;
		m_is = null;
		...
		if(systemId != null && m_externalEntity != null)
		{
			systemId = new URL(m_externalEntity.getURL(), systemId).toString();
		}
		else if(m_baseURI != null)
		{
			try
			{
				systemId = new URL(new URL(m_baseURI), systemId).toString();
			}
			catch(Exception e)
			{}
		}
		...
		if(systemId != null && m_handler != null)
		{
			Object input = m_handler.resolveEntity(publicId, systemId);
			if(input != null)
			{
		...
		if(m_handler != null)
		{
			if(systemId != null)
			{
				m_handler.startExternalEntity(systemId);
			}
			else
			{
				m_handler.startExternalEntity("[external stream]");
			}
		}
		...
		else
		{
			...
			// Set the new status
			m_sourceType = INPUT_EXTERNAL;
			url = new URL(systemId);

			m_externalEntity = url.openConnection();
			m_externalEntity.connect();
			m_is = m_externalEntity.getInputStream();
		}
		...
		{
			m_is = new BufferedInputStream(m_is);
		}

		// Attempt to detect the encoding.
		if(encoding == null && m_externalEntity != null)
		{
			encoding = m_externalEntity.getContentEncoding();
		}
	...
	void pushInput(String ename) throws java.lang.Exception
	{
		...
		if(ename != null)
		{
			Enumeration entities = m_entityStack.elements();
		...
		m_entityStack.push(ename);

		// Don't bother if there is no input.
		if(m_sourceType == INPUT_NONE)
		{
			return;
		}
		...
		// input source.
		input[0] = new Integer(m_sourceType);
		input[1] = m_externalEntity;
		input[2] = m_readBuffer;
		input[3] = new Integer(m_rbpos);
		input[4] = new Integer(m_rblen);
		input[5] = new Integer(m_line);
		input[6] = new Integer(m_encoding);
		input[7] = new Integer(m_rboverflow);
		input[8] = m_is;
		input[9] = new Integer(m_currentByteCount);
		input[10] = new Integer(m_column);
		input[11] = m_reader;
	...
	void popInput() throws java.lang.Exception
	{
		...
		switch(m_sourceType)
		{
			...
			case INPUT_EXTERNAL:
				dataBufferFlush();
				if(m_handler != null && m_externalEntity != null)
				{
					m_handler.endExternalEntity(m_externalEntity.getURL().toString());
				}
				break;
			case INPUT_STREAM:
				dataBufferFlush();
				if(m_baseURI != null)
				{
					if(m_handler != null)
					{
						m_handler.endExternalEntity(m_baseURI);
					}
				}
				break;
			case INPUT_READER:
		...
		{
			input = (Object[]) m_inputStack.pop();
			m_entityStack.pop();
		}

		m_sourceType = ((Integer) input[0]).intValue();
		m_externalEntity = (URLConnection) input[1];
		m_readBuffer = (char[]) input[2];
		m_rbpos = ((Integer) input[3]).intValue();
		m_rblen = ((Integer) input[4]).intValue();
		m_line = ((Integer) input[5]).intValue();
		m_encoding = ((Integer) input[6]).intValue();
		m_rboverflow = ((Integer) input[7]).intValue();
		m_is = (InputStream) input[8];
		m_currentByteCount = ((Integer) input[9]).intValue();
		m_column = ((Integer) input[10]).intValue();
		m_reader = (Reader) input[11];
	}
	...
	void read8bitEncodingDeclaration() throws java.lang.Exception
	{
		...
		while(true)
		{
			...
			if(m_readBuffer.length == m_rblen)
			{
	...
	void readDataChunk() throws java.lang.Exception
	{
		...
		if(m_rboverflow > -1)
		{
			m_readBuffer[0] = (char) m_rboverflow;
			m_rboverflow = -1;
			m_rbpos = 1;
			m_sawCR = true;
		}
		...
		if(m_sourceType == INPUT_READER)
		{
			count = m_reader.read(m_readBuffer, m_rbpos, READ_BUFFER_MAX - 1);
			if(count < 0)
			{
				m_rblen = -1;
			}
		...
		count = m_is.read(m_rawReadBuffer, 0, READ_BUFFER_MAX);
	...
	void filterCR()
	{
		...
		loop : for(i = 0, j = 0; j < m_rblen; i++, j++)
		{
			switch(m_readBuffer[j])
			{
				case '\r':
					if(j == m_rblen - 1)
					{
	...
	void copyUtf8ReadBuffer(int count) throws java.lang.Exception
	{
		...
		while(i < count)
		{
			...
			if((b1 & 0x80) == 0)
			{
				...
				m_readBuffer[j++] = (char) b1;
			}
			else if((b1 & 0xe0) == 0xc0)
			{
				// 2-byte sequence: 00000yyyyyxxxxxx = 110yyyyy 10xxxxxx
				m_readBuffer[j++] = (char) (((b1 & 0x1f) << 6) | getNextUtf8Byte(i++, count));
			}
			else if((b1 & 0xf0) == 0xe0)
			{
				// 3-byte sequence: zzzzyyyyyyxxxxxx = 1110zzzz 10yyyyyy 10xxxxxx
				m_readBuffer[j++] = (char) (((b1 & 0x0f) << 12) | (getNextUtf8Byte(i++, count) << 6) | getNextUtf8Byte(i++, count));
			}
			else if((b1 & 0xf8) == 0xf0)
			{
				...
				m_readBuffer[j++] = (char) (0xd800 | ((((b1 & 0x07) << 2) | ((b2 & 0x30) >> 4) - 1) << 6) | ((b2 & 0x0f) << 2) | ((b3 & 0x30) >> 4));
				m_readBuffer[j++] = (char) (0xdc | ((b3 & 0x0f) << 6) | b4);
	...
	{
		int i, j;
		for(i = 0, j = m_rbpos; i < count; i++, j++)
		{
			m_readBuffer[j] = (char) (m_rawReadBuffer[i] & 0xff);
			if(m_readBuffer[j] == '\r')
			{
				m_sawCR = true;
			}
	...
	void copyUcs2ReadBuffer(int count, int shift1, int shift2) throws java.lang.Exception
	{
		...
		{
			m_readBuffer[j++] = (char) (((m_rawReadBuffer[i] & 0xff) << shift1) | ((m_rawReadBuffer[i + 1] & 0xff) << shift2));
			if(m_readBuffer[j - 1] == '\r')
			{
				m_sawCR = true;
			}
	...
	void copyUcs4ReadBuffer(int count, int shift1, int shift2, int shift3, int shift4) throws java.lang.Exception
	{
		...
		{
			value = (((m_rawReadBuffer[i] & 0xff) << shift1) | ((m_rawReadBuffer[i + 1] & 0xff) << shift2)
					| ((m_rawReadBuffer[i + 2] & 0xff) << shift3) | ((m_rawReadBuffer[i + 3] & 0xff) << shift4));
			if(value < 0x0000ffff)
			{
				m_readBuffer[j++] = (char) value;
				if(value == '\r')
				{
					m_sawCR = true;
				}
			...
			{
				m_readBuffer[j++] = (char) (0xd8 | ((value & 0x000ffc00) >> 10));
				m_readBuffer[j++] = (char) (0xdc | (value & 0x0003ff));
			}
			else
			{
	...
	void encodingError(String message, int value, int offset) throws java.lang.Exception
	{
		...
		m_handler.error(message, uri, -1, offset + m_currentByteCount);
	}
	...
	void initializeVariables()
	{
		...
		m_dataBufferPos = 0;
		m_dataBuffer = new char[DATA_BUFFER_INITIAL];
		m_nameBufferPos = 0;
		m_nameBuffer = new char[NAME_BUFFER_INITIAL];

		// Set up the DTD hash tables
		m_elementInfo = new Hashtable();
		m_entityInfo = new Hashtable();
		m_notationInfo = new Hashtable();

		// Set up the variables for the current
		// element context.
		m_currentElement = null;
		m_currentElementContent = CONTENT_UNDECLARED;

		// Set up the input variables
		m_sourceType = INPUT_NONE;
		m_inputStack = new Stack();
		m_entityStack = new Stack();
		m_externalEntity = null;
		m_tagAttributePos = 0;
		m_tagAttributes = new String[100];
		m_rawReadBuffer = new byte[READ_BUFFER_MAX];
		m_rboverflow = -1;

		m_context = CONTEXT_NONE;

		m_symbolTable = new Object[SYMBOL_TABLE_LENGTH];
	}
	...
	// I/O information.
	//
	private Reader m_reader; // current reader

	private InputStream m_is; // current input stream

	private int m_line; // current line number

	private int m_column; // current column number

	private int m_sourceType; // type of input source

	private Stack m_inputStack; // stack of input soruces

	private URLConnection m_externalEntity; // current external entity

	private int m_encoding; // current character encoding.

	private int m_currentByteCount; // how many bytes read from current source.
	...
	private int m_rboverflow; // overflow character from last data chunk.
