...
public class DemoWeekAgenda extends UrlPage {
	...
	public void createContent() throws Exception {
		...
		IItemRenderer<ScheduleItem> care = new DefaultScheduleItemRenderer<ScheduleItem>() {
			@Override
			public void render(WeekAgendaComponent<ScheduleItem> age, NodeContainer root, ScheduleItem si) throws Exception {
	...
	public void newAppt(Date dt, long dur) throws Exception {
		...
		FormBuilder f = new FormBuilder(floater);
		final DateInput di = new DateInput();
		di.setValue(dt);
		f.label("Afspraaktijd").mandatory(true).control(di);
		final Text2<Long> dc = new Text2<Long>(Long.class);
		dc.setConverter(new SecondDurationConverter());
		dc.setValue(Long.valueOf(dur / 1000));
		f.label("Duur").mandatory().control(dc);

		final Text2<String> sub = new Text2<String>(String.class);
		f.label("Omschrijving").mandatory().control(sub);
