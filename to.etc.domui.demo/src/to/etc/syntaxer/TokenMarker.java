...
public class TokenMarker {
	...
	private boolean checkDelegateEnd(ParserRule rule) {
		...
		if(b && !tempEscaped) {
			...
			m_tokenHandler.handleToken(m_line, (m_context.inRule.action & ParserRule.EXCLUDE_MATCH) == ParserRule.EXCLUDE_MATCH ? m_context.rules.getDefault() : m_context.inRule.token, m_pos
				- m_line.offset, m_pattern.count, m_context);
		...
		//{{{ Handle start of rule
			...
			switch(checkRule.action & ParserRule.MAJOR_ACTIONS){
				...
				case ParserRule.EOL_SPAN:
					...
					byte tokenType = ((checkRule.action & ParserRule.EXCLUDE_MATCH) == ParserRule.EXCLUDE_MATCH ? m_context.rules.getDefault() : checkRule.token);
				...
				case ParserRule.MARK_FOLLOWING:
					m_tokenHandler.handleToken(m_line, (checkRule.action & ParserRule.EXCLUDE_MATCH) == ParserRule.EXCLUDE_MATCH ? m_context.rules.getDefault() : checkRule.token, m_pos
						- m_line.offset, m_pattern.count, m_context);
				...
				case ParserRule.MARK_PREVIOUS:
					...
					if((checkRule.action & ParserRule.EXCLUDE_MATCH) == ParserRule.EXCLUDE_MATCH) {
						...
						m_tokenHandler.handleToken(m_line, checkRule.token, m_lastOffset - m_line.offset, m_pos - m_lastOffset + m_pattern.count, m_context);
					}
