...
public class AnimGifEncoder {
	...
	public void encode() throws IOException {
		...
		m_htab = new int[HSIZE];
		m_codetab = new int[HSIZE];
		accum = new byte[256];
	...
	private void genCompressed(AnIma a, int initcodesz) throws IOException {
		...
		m_free_ent = 0;
		m_clear_flg = false;
		m_maxbits = BITS; // user settable max # bits/code
		m_maxmaxcode = 1 << BITS; // should NEVER generate this code
		a_count = 0;
		m_g_init_bits = initcodesz; // Initial #of bits

		// Set up the necessary values
		m_clear_flg = false;
		m_n_bits = m_g_init_bits;
		m_maxcode = MAXCODE(m_n_bits);
		m_clearCode = 1 << (initcodesz - 1);
		m_eofCode = m_clearCode + 1;
		m_free_ent = m_clearCode + 2;
		char_init();

		hshift = 0;
		for(fcode = m_hsize; fcode < 65536; fcode *= 2)
			++hshift;
		...
		{
			c = m_curr_pixels[m_px_ix++] & 0xff; // Get next pixel value,

			fcode = (c << m_maxbits) + ent;
			i = (c << hshift) ^ ent; // xor hashing

			if(i >= m_htab.length || i < 0)
				System.out.println("SHIT!");

			if(m_htab[i] == fcode) {
				ent = m_codetab[i];
				continue;
			} else if(m_htab[i] >= 0) // non-empty slot
			{
	...
	int					m_maxbits		= BITS;		// user settable max # bits/code

	int					m_maxcode;						// maximum code, given n_bits

	int					m_maxmaxcode	= 1 << BITS;	// should NEVER generate this code
	...
	int			m_hsize		= HSIZE;																																	// for dynamic table sizing

	int			m_free_ent	= 0;																																		// first unused entry
	...
	void output(int code) throws IOException {
		...
		// $$Rewrote if (JAL)
		if(m_clear_flg) {
			m_maxcode = MAXCODE(m_n_bits = m_g_init_bits);
			m_clear_flg = false;
	...
	private void outputEOF() throws IOException {
		output(m_eofCode); // Actually output the code
